...
interface VbaVariablesQuizPageProps {
  params: { locale: Locale };
}

export default async function VbaVariablesQuizPage({ params: { locale } }: VbaVariablesQuizPageProps) {
  const dict = await getDictionary(locale);
  const t = dict.quizFormulasPage; // Can reuse the same translations for buttons, results, etc.
  const quizData = dict.vbaVariablesQuiz;
