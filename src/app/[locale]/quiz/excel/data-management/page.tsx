...
interface DataManagementQuizPageProps {
  params: { locale: Locale };
}

export default async function DataManagementQuizPage({ params: { locale } }: DataManagementQuizPageProps) {
  const dict = await getDictionary(locale);
  const t = dict.quizFormulasPage; // Can reuse the same translations for buttons, results, etc.
  const quizData = dict.excelDataManagementQuiz;
